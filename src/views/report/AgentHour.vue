  ...
  <div class="page-card">
    ...
    <div class="table-wrapper">
      <el-table :data="list" v-loading="loading" size="small" style="width:100%">
        <el-table-column label="统计时间" width="170">
...
import { getAgentHourStatList } from '@/api/config'

const loading = ref(false)
const list = ref([])
const searchForm = reactive({ agentKey: '', statTime: null })
...
const loadData = async () => {
  ...
  try {
    ...
    if (searchForm.statTime) query.statTime = Math.floor(searchForm.statTime / 1000)
    const params = { pageNum: pagination.currentPage, pageSize: pagination.pageSize,
      query: JSON.stringify(query) }
    const res = await getAgentHourStatList(params)
...
const handleReset = () => { searchForm.agentKey = ''; searchForm.statTime = null; pagination.currentPage = 1; loadData() }
